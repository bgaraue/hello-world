...
class KnnTest(unittest.TestCase):
    ...
    def test_minkowski_distance(self):
        ...
        knn = Knn(n_neighbors=3, p=5) # Llamada al método __init__() de la clase Knn
        knn.fit(np.array(little_X), little_Y)
    ...
    def test_input_dimension(self):
        """Test to check that we raise an exception if X and y dimensions are not consistent"""
        knn = Knn(n_neighbors=3)
    ...
    def test_distance_weight_with_0(self):
        """Test to see that _distance_weights return a weight of 1/d for each distance"""
        knn = Knn(n_neighbors=3)
    ...
    def test_k_1_distance_manhattan(self):
        """Test to compare our knn with Sklearn knn when k=1 and distance is manhattan"""
        knn = KNeighborsClassifier(n_neighbors=1, metric="manhattan")
        knn.fit(X_train, y_train)
        prediction = knn.predict(X_test)

        knn2 = Knn(n_neighbors=1, metric="manhattan")
        knn2.fit(X_train, y_train)
        prediction2 = knn2.predict(X_test)

        assert np.alltrue(prediction == prediction2), "Error testing knn (manhattan) with k=1"
    ...
    def test_distance_weight_2(self):
        ...
        knn2 = Knn(n_neighbors=5, weights='uniform')
        knn2.fit(X_train, y_train)
        prediction2 = knn2.predict(X_test)
